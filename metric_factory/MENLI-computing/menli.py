...
def compute_menli(df:pd.DataFrame) -> pd.DataFrame:
    scorer_BERT = MENLI(direction='avg', formula='e', src=False, nli_weight=0.3, combine_with='BERTScore-F')
    scorer_Mover = MENLI(direction='avg', formula='e', src=False, nli_weight=0.3, combine_with='MoverScore')
    ...
    df_MENLI = pd.DataFrame({'Model': list_model, 'MENLI_x_BERT': list_BERT, 'MENLI_x_Mover': list_Mover})
    return df_MENLI
    ...
    def get_model(self):
        if self.model_type == 'R':
            if not self.cross_lingual:
                ...
                model = AutoModelForSequenceClassification.from_pretrained('ynie/roberta-large-snli_mnli_fever_anli_R1_R2_R3-nli', num_labels=3)
            else:
            ...
            if self.cross_lingual:
                ...
                model = AutoModelForSequenceClassification.from_pretrained("MoritzLaurer/mDeBERTa-v3-base-mnli-xnli", num_labels=3)
            else:
                tokenizer = AutoTokenizer.from_pretrained("microsoft/deberta-large-mnli", use_fast=False)
