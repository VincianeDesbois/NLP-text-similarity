...
import gdown
from MENLI_main.experiments.metrics.bert_score.scorer import BERTScorer
from MENLI_main.experiments.metrics.moverscore_re import MoverScorer


def compute_menli(df: pd.DataFrame) -> pd.DataFrame:
    scorer_BERT = MENLI(
        direction="avg",
        formula="e",
        src=False,
        nli_weight=0.3,
        combine_with="BERTScore-F",
    )
    scorer_Mover = MENLI(
        direction="avg",
        formula="e",
        src=False,
        nli_weight=0.3,
        combine_with="MoverScore",
    )
    list_model = df["Model"].value_counts().index
    list_BERT = []
    list_Mover = []
    for mod in list_model:
        df_mod = df[df["Model"] == mod].reset_index()
        refs = df_mod.Human.values.tolist()
        hyps = df_mod.Story.values.tolist()
        scores_BERT = scorer_BERT.score_all(srcs=[], refs=refs, hyps=hyps)
        scores_Mover = scorer_Mover.score_all(srcs=[], refs=refs, hyps=hyps)
        list_BERT.append(str(scores_BERT))
        list_Mover.append(str(scores_Mover))

    df_MENLI = pd.DataFrame(
        {"Model": list_model, "MENLI_x_BERT": list_BERT, "MENLI_x_Mover": list_Mover}
    )
    return df_MENLI
...
class MENLI:
        ...
        with torch.no_grad():
            if direction in ["rh", "avg"]:
                ...
                for ref, hyp in zip(refs, hyps):
                    (
                        ...
                        token_type_ids,
                        attention_mask,
                    ) = self.collate_input_features(ref, hyp)
                    logits = self._model(
                        input_ids,
                ...
                if self.model_type == "D" and not self.cross_lingual:
                    probs_rh["e"], probs_rh["n"], probs_rh["c"] = (
                        concatenated[:, 2],
                        concatenated[:, 1],
                        concatenated[:, 0],
                    )
            ...
            if direction in ["hr", "avg"]:
                ...
                for ref, hyp in zip(refs, hyps):
                    (
                        ...
                        token_type_ids,
                        attention_mask,
                    ) = self.collate_input_features(hyp, ref)
                    logits = self._model(
                        input_ids,
                        attention_mask=attention_mask,
                        token_type_ids=token_type_ids,
                ...
                concatenated = np.concatenate(probs, 0)
                if self.model_type == "D" and not self.cross_lingual:
                    probs_hr["e"], probs_hr["n"], probs_hr["c"] = (
                        concatenated[:, 2],
                        concatenated[:, 1],
                        concatenated[:, 0],
                    )
            ...
            if direction == "rh":
                ...
                for p in probs_hr.keys():
                    probs_avg[p] = [
                        (s1 + s2) / 2.0 for s1, s2 in zip(probs_rh[p], probs_hr[p])
                    ]
    ...
    def combine(self, metric_scores, nli_scores, nli_weight=None):
        ...
        final_scores = [
            nli_weight * n + (1 - nli_weight) * m
            for n, m in zip(norm_nli_scores, norm_metric_scores)
        ]
        return final_scores
    ...
    def combine_all(self):
        ...
        final_scores = [
            self.nli_weight * n + (1 - self.nli_weight) * m
            for n, m in zip(norm_nli_scores, norm_metric_scores)
        ]
        ...
        if self.model_type == "R":
            if not self.cross_lingual:
                ...
                model = AutoModelForSequenceClassification.from_pretrained(
                    "ynie/roberta-large-snli_mnli_fever_anli_R1_R2_R3-nli", num_labels=3
                )
            else:
                tokenizer = AutoTokenizer.from_pretrained(
            ...
            if self.cross_lingual:
                tokenizer = AutoTokenizer.from_pretrained(
                    "MoritzLaurer/mDeBERTa-v3-base-mnli-xnli", use_fast=False
                )
                model = AutoModelForSequenceClassification.from_pretrained(
                    "MoritzLaurer/mDeBERTa-v3-base-mnli-xnli", num_labels=3
                )
            else:
                tokenizer = AutoTokenizer.from_pretrained(
                    "microsoft/deberta-large-mnli", use_fast=False
                )
                model = AutoModelForSequenceClassification.from_pretrained(
                    "microsoft/deberta-large-mnli", num_labels=3
                )
        model.eval()
